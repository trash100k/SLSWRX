...
// Builds the Express app exposing the 6 /api endpoints. Shared by the local
// dev server (server.ts) and the Vercel serverless function (api/[...path].ts).
export function createApiApp(): express.Express {
  const app = express();
  app.use(express.json({ limit: "1mb" }));

  // Claude client. Empty-string fallback keeps the app bootable without a key
  // (calls 401 until ANTHROPIC_API_KEY is set) rather than throwing here.
